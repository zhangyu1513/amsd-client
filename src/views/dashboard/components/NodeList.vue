      ...
      <div v-for="node in data" :key="node.ID" class="p-3 rounded-lg dark:bg-[#1d1d1d]">
        <div class="flex items-center gap-2">
          ...
          <div class="flex-1 min-w-0 px-2">
            <div class="text-xs text-gray-400 dark:text-gray-500 truncate" :title="node.Workspace">
              {{ node.Workspace }}
            </div>
            <div class="text-xs text-gray-400 dark:text-gray-500 truncate" :title="node.ExePath">
              {{ node.ExePath }}
            </div>
          </div>
          <!-- 右侧：状态信息 -->
          <div class="flex items-center gap-3 shrink-0">
            ...
            <div class="w-14 text-center">
              <div class="text-xs text-gray-500 dark:text-gray-400">线程</div>
